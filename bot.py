...
# ============================
# Ark
@bot.tree.command(name="ark_cleardinoes", description="Clears all wild dinos if the server is running.")
async def cleardinoes(interaction: discord.Interaction):
    ...
    if instance.running:
        ...
        await interaction.followup.send("⚠️ The Ark server is offline, and the game server is not running.",
                                        ephemeral=True)


# ============================
# BeamMp
@bot.tree.command(name="beam_map", description="Changes the current map of the server")
async def beam_map(interaction: discord.Interaction):
    await interaction.response.defer(thinking=True, ephemeral=True)
...
async def status_autocomplete(interaction: discord.Interaction, current: str):
    options = [
        app_commands.Choice(name="Minecraft", value=MinecraftInstanceName),
        app_commands.Choice(name="Ark", value=ArkInstanceName),
        app_commands.Choice(name="CounterStrike 2", value=CounterstrikeInstanceName),
        app_commands.Choice(name="Beam MP Drive", value=BeamMPInstanceName),
    ]
    return [option for option in options if current.lower() in option.name.lower()]
